...
public class PlantPotBlock extends HorizontalFacingBlock {
    ...
    @Override
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {BlockPos up = pos.up();if (world.getBlockState(up).isOf(ModBlocks.TOMATO_CROP) || world.getBlockState(up).isOf(ModBlocks.GARLICS) || world.getBlockState(up).isOf(ModBlocks.ROSEMARY_BUSH)) {world.setBlockState(pos, state.with(MOISTURE, 7), 2);} else {super.randomTick(state, world, pos, random);}}
    @Nullable
    @Override
