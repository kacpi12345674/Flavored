...
public class CauliflowerBlock extends GourdBlock {
    ...
    public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
        ...
        if (itemStack.isOf(Items.SHEARS)) {
            if (!world.isClient) {
                ...
                world.setBlockState(pos, (BlockState) ModBlocks.CARVED_CAULIFLOWER.getDefaultState().with(CarvedPumpkinBlock.FACING, direction2), 11);
                ItemEntity itemEntity = new ItemEntity(world, (double)pos.getX() + 0.5 + (double)direction2.getOffsetX() * 0.65, (double)pos.getY() + 0.1, (double)pos.getZ() + 0.5 + (double)direction2.getOffsetZ() * 0.65, new ItemStack(ModItems.CAULIFLOWER_SEEDS, 4));
    ...
    public StemBlock getStem() {
        return (StemBlock)ModBlocks.CAULIFLOWER_STEM;
    }

    public AttachedStemBlock getAttachedStem() {
        return (AttachedStemBlock)ModBlocks.ATTACHED_CAULIFLOWER_STEM;
    }
}
